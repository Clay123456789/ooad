package com.ooad.system_management.Utils;
...
public class FtpUtil {
	 ...
	 * 上传文件到ftp服务器
	 */
	public static List<String> ftpUpload2(List<File> files, String ftpUrl, int ftpPort,
                                          String ftpUsername, String ftpPassword, String ftpRemotePath) {
		List<String> result = null;
		try {
			boolean isConnection = ftp.openConnection(ftpUrl, ftpPort, ftpUsername, ftpPassword);
			if (isConnection) {
				result = ftp.upload2(ftpRemotePath, files);
				if (result!=null) {
					log.info("文件上传成功！");
				} else {
					log.info("文件上传失败！");
	 ...
	 *             异常
	 */
	public List<String> upload2(String remotePath, List<File> files) throws IOException {
